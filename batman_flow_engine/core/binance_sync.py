...
def show_balances():
    ...
    if not balances:
        print(f"  ❌ No balances found (check API key)")
        return
...
def show_and_import_trades():
    ...
    balances = get_account_balances()
    symbols = [f"{b['asset']}USDT" for b in balances 
               if b["asset"] not in ("USDT", "BUSD", "USD", "MXN", "BNB")
               and b["total"] > 0]
    
    if not symbols:
        print(f"  No assets to fetch trades for")
        return
...
def auto_hodl():
    ...
    print(f"  ⚠️  Targets NOT set — use Oracle to add TP1/TP2/TP3/stop:")
    print(f"     python tools/command_center.py --hodl")
